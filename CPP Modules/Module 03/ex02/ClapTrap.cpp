...
std::ostream &operator<<(std::ostream &stream, ClapTrap &clapTrap)
{
    ...
    << ANSI_COLOR_BRED << " • HP: " << ANSI_RESET << clapTrap.getHitpoints()
    << ANSI_COLOR_BYELLOW << " • EP: " << ANSI_RESET << clapTrap.getEnergyPoints()
    << ANSI_COLOR_BRED << " • AD: " << ANSI_RESET << clapTrap.getAttackDamage();
	return (stream);
}
